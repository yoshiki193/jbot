import discord
from discord.ext import commands
from repositories.data_repository import DataRepository

class CounterService:
    def __init__(self, repo: DataRepository):
        self.repo = repo

    def add(self, member_id: int, guild_id: int):
        self.repo.set_counter_users(guild_id, member_id)
    
    def get_users(self, guild_id: int):
        return self.repo.get_counter_users(guild_id)
    
    def get_ban_users(self, guild_id: int):
        return self.repo.get_ban_users(guild_id)
    
    def get_last_message_id(self, guild_id: int):
        return self.repo.get_last_message_id(guild_id)

    def set_last_message_id(self, guild_id: int, message_id: int):
        self.repo.set_last_message_id(guild_id, message_id)
    
    def get_send_channel_id(self, guild_id: int):
        return self.repo.get_send_channel_id(guild_id)
    
    def is_counter_update_message(self, message: discord.Message, bot: commands.Bot):
        if message is None or message.guild is None or message.channel is None:
            return False

        send_channel_id = self.get_send_channel_id(message.guild.id)
        if send_channel_id is None:
            return False

        if (message.author != bot or "updated" in message.content) and message.channel.id == send_channel_id:
            return True

        return False